...
// Column mapping from Excel headers to API payload keys
...
function parseTabSeparated(text: string): CreateJabatanFungsionalRefPayload[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    optionals.forEach((k) => {
      if (row[k]) (item as Record<string, string>)[k] = row[k];
    });
...
type DetailPanelProps = { item: SiformenJabatanFungsionalRef; onClose: () => void };
          ...
          {fields.map(({ label, value }) =>
            value ? (
              <div key={label}>
                ...
                <p className="mt-0.5 text-sm text-gray-800 dark:text-gray-200 whitespace-pre-line">
                  {value}
                </p>
              </div>
            ) : null,
...
function ImportModal({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  ...
  function handleParse() {
    ...
    setResult(null);
    if (!csvText.trim()) {
      setParseError('Tempel data terlebih dahulu');
      return;
    }
    const parsed = parseTabSeparated(csvText);
    if (parsed.length === 0) {
      setParseError(
        'Tidak ada baris valid. Pastikan format sesuai: header tab-separated di baris pertama.',
      );
      return;
    }
    setPreview(parsed);
      ...
      <div className="relative z-10 w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-xl bg-white shadow-2xl dark:bg-gray-900">
        ...
        <div className="p-6 space-y-4">
          <div className="rounded-lg bg-blue-50 p-3 text-sm text-blue-800 dark:bg-blue-950 dark:text-blue-200">
            <strong>Format:</strong> Salin langsung dari Excel (tab-separated). Baris pertama harus
            header. Kolom yang dikenali:{' '}
            <span className="italic">
              Nama Jabatan Fungsional, Jenjang, Kategori, Awal, Puncak, Golongan Ruang Awal,
              Rumpun Jabatan, Ruang Lingkup, Pengisian ASN, Instansi Pembina, Dasar Hukum, Tugas
              Jabatan, Pendidikan_Pengangkatan_Pertama, Pendidikan_Perpindahan, Peraturan Presiden
              Tentang Tunjangan, Besaran Tunjangan
            </span>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Salin dan tempel di sini..."
              className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100"
            />
          </div>
          ...
          <div className="flex gap-2">
            <button
              onClick={handleParse}
              className="inline-flex items-center gap-1.5 rounded-lg bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600"
            >
          ...
          {result && (
            <div className="rounded-lg bg-green-50 p-3 text-sm text-green-800 dark:bg-green-950 dark:text-green-200">
              Import selesai: <strong>{result.created}</strong> baru ditambahkan,{' '}
              <strong>{result.skipped}</strong> diperbarui.
            </div>
          )}
              ...
              <div className="overflow-x-auto rounded-lg border dark:border-gray-700">
                <table className="w-full text-xs">
                  <thead className="bg-gray-50 dark:bg-gray-800">
                    <tr>
                      {['Nama Jabatan', 'Jenjang', 'Kategori', 'Rumpun', 'Instansi Pembina', 'Pengisian ASN'].map(
                        (h) => (
                  ...
                  <tbody>
                    {displayRows.map((row, i) => (
                      <tr key={i} className="border-t dark:border-gray-700">
                        ...
                        <td className="px-3 py-1.5 text-gray-600 dark:text-gray-400 max-w-[120px] truncate">
                          {row.rumpunJabatan ?? '-'}
                        </td>
                        <td className="px-3 py-1.5 text-gray-600 dark:text-gray-400 max-w-[140px] truncate">
                          {row.instansiPembina ?? '-'}
                        </td>
                        <td className="px-3 py-1.5 text-gray-600 dark:text-gray-400 whitespace-nowrap">
                          {row.pengisianAsn ?? '-'}
              ...
              {preview.length > 10 && (
                ...
                >
                  {showAll ? (
                    ...
                    <>
                      <ChevronDown className="h-3 w-3" /> Tampilkan semua {preview.length} baris
                    </>
                  )}
                </button>
              )}
...
export function SiformenJabatanFungsionalRefPage() {
  ...
  const [items, setItems] = useState<SiformenJabatanFungsionalRef[]>([]);
  ...
  const [rumpunOptions, setRumpunOptions] = useState<string[]>([]);
  ...
  const load = useCallback(() => {
    ...
    siformenApi
      ...
      .catch((caught) => {
        if (mounted) setError(caught instanceof Error ? caught.message : 'Gagal memuat data');
      })
      .finally(() => {
  ...
  useEffect(() => {
    siformenApi
      .getFilterOptions()
      .then((opts) => {
        setRumpunOptions(opts.rumpunJabatan);
        setInstansiOptions(opts.instansiPembina);
      })
      .catch(() => {});
  }, []);

  async function handleDelete(item: SiformenJabatanFungsionalRef) {
    if (!confirm(`Hapus "${item.namaJabatan} — ${item.jenjang}"?`)) return;
    setActionLoading(item.id);
    try {
      await siformenApi.deleteJabatanFungsionalRef(item.id);
      load();
    } finally {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (item: SiformenJabatanFungsionalRef) => (
        <StatusBadge
          value={item.kategori}
          tone={item.kategori === 'KETERAMPILAN' ? 'warning' : 'info'}
        />
      ),
    },
    {
      ...
      <SectionCard>
        <FilterBar>
          ...
          <select
            ...
            className="rounded-lg border border-gray-300 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100"
          >
          ...
          {rumpunOptions.length > 0 && (
            <select
              value={rumpunJabatan}
              onChange={(e) => { setRumpunJabatan(e.target.value); setPage(1); }}
              className="rounded-lg border border-gray-300 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100"
            >
          ...
          <ActionButton icon={RefreshCcw} label="Refresh" onClick={load} variant="ghost" />
        </FilterBar>
        ...
        {loading ? (
          ...
          <>
            <div className="mb-2 text-sm text-gray-500 dark:text-gray-400">
              {total} jabatan fungsional ditemukan
            </div>
            <DataTable
              columns={columns}
              data={items}
              emptyMessage={
                q || kategori || rumpunJabatan
                  ? 'Tidak ada jabatan yang cocok dengan filter'
                  : 'Belum ada data referensi. Gunakan Import CSV untuk memuat data BKN.'
              }
            />
            {total > 25 && (
              <div className="mt-4 flex items-center justify-between text-sm text-gray-600 dark:text-gray-400">
                <span>
                  Halaman {page} dari {Math.ceil(total / 25)}
                </span>
                <div className="flex gap-2">
                  <button
                    disabled={page === 1}
