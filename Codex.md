Phase 9B kita isi dengan:

Backend auto event worker — pending event diproses otomatis tanpa harus manual POST /events/process-pending.
Frontend notification bell di topbar.
Unread badge, dropdown notifikasi, mark read, mark all read, dan klik notifikasi menuju actionUrl.

Struktur frontend topbar saat ini ada di apps/web/src/components/workspace/layout.tsx, jadi notifikasi paling tepat ditempatkan di kanan topbar sebelum user/logout.
API client frontend juga sudah punya get, post, patch, upload, dan download, jadi cukup dipakai untuk endpoint notifications.

1. Backend — buat file baru
api/src/modules/events/event-worker.service.ts
import {
  Inject,
  Injectable,
  Logger,
...
2. Backend — replace file
api/src/modules/events/events.module.ts
...
Tambahkan blok type ini di bagian bawah file:
...
4. Frontend — replace penuh layout
apps/web/src/components/workspace/layout.tsx
...
function NotificationBell() {
  ...
  return (
    <div ref={wrapperRef} className="relative">
      ...
      {open ? (
        <div className="absolute right-0 mt-3 w-[360px] overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-xl">
          <div className="flex items-center justify-between gap-3 border-b border-zinc-100 px-4 py-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                className="inline-flex items-center gap-1 rounded-lg border border-zinc-200 px-2.5 py-1.5 text-xs font-semibold text-zinc-700 hover:bg-zinc-50 disabled:opacity-50"
            ...
            {items.length > 0 ? (
              <div className="divide-y divide-zinc-100">
                {items.map((notification) => (
                  <button
                    key={notification.id}
                    className="grid w-full gap-1 px-4 py-3 text-left transition-colors hover:bg-zinc-50"
                  ...
                  >
                    ...
                    {notification.case ? (
                      <div className="mt-1 rounded-lg bg-zinc-50 px-2.5 py-2 text-xs text-zinc-600">
...
Kalau ingin interval lebih cepat saat development, isi .env:

EVENT_WORKER_ENABLED=true
EVENT_WORKER_INTERVAL_MS=10000
6. Validasi Frontend
...
Login.
Submit SIPENSIUN case / start task / complete task.
Tunggu maksimal 30 detik.
Lihat icon bell kanan atas.
Pastikan badge merah muncul.
Klik bell.
Klik notifikasi.
Notifikasi berubah read dan redirect ke actionUrl.